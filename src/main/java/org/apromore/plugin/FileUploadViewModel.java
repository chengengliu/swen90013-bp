...
public class FileUploadViewModel {
    ...
    public void onFileUpload() {
        ...
        if (media != null) {
            ...
            List<String> files = fileHandlerService.getFilePathList();
            if (returnMessage.equals("Upload Success") && files.size() > 0) {
                String lastPath = files.get(files.size() - 1);
                addFileToUIList(lastPath);
            }
        } else {
            Messagebox.show(
                NULL_UPLOAD_MESSAGE,
